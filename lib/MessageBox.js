...
const replacementStrings = ['genericName', 'name', 'type'];
const replacements = replacementStrings.reduce((result, replacement) => ({
  ...result,
  [replacement]: new RegExp(`{{${replacement}}}`, 'g'),
}), {});

const compiler = (message) => (context) =>
  Object.keys(replacements).reduce((result, replacement) =>
    result.replace(replacements[replacement], context[replacement]), message
  );
